...
check_env() {
    if [ -z "${ADMIN_IDP_GROUP:-}" ] || [ -z "${MEMBER_IDP_GROUP:-}" ]; then
        echo "ERROR: environment variables ADMIN_IDP_GROUP and MEMBER_IDP_GROUP must be set, exist..."
	exit 1
    fi
}
...
update_default_group() {
   ...
   jq --arg adminGroup "${ADMIN_IDP_GROUP}" \
      ...
      '.roleBindings[].subjects[] |= if .kind=="GROUP"
        then
          ...
          .
        end' $rolebindings > temp.out
    mv temp.out $rolebindings
}
